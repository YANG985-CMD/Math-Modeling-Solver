...
- `data_mode=formal`: require real inputs and real execution for empirical claims.
- `data_mode=demo`: allow synthetic or example data, but label every output as illustrative.
- `gate_status=blocked`: preserve the plan and list exact missing inputs without fabricating results.
- `workflow_stage`: use `explore`, `validate`, or `deliver` to control which checks run now.
- `result_status`: use `draft`, `validated`, `frozen`, or `manuscript` for paper-facing evidence.

Data mode controls evidence truthfulness. Gate status controls whether work may advance. Delivery profile controls packaging:
...
Failure action: set the gate status to `blocked` or request data. Switching to demo data requires explicit consent.
...
- score-gap diagnosis when a scored optimization contract is additive and normalized;
- hard-constraint action masks for constructive or policy search, plus an exact-transition comparison when a surrogate simulator is used;
- registered experiment budgets and stop rules before repeated tuning campaigns.
...
- the experiment has reached <code>independently_validated</code>, which maps to project result status <code>validated</code>, before it is frozen;
