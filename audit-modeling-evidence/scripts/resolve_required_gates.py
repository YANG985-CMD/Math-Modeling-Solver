...
STAGE_ORDER = {"explore": 0, "validate": 1, "deliver": 2}
LEGACY_STAGE_ALIASES = {"candidate": "validate", "delivery": "deliver"}
TABULAR_FAMILIES = {
    "prediction",
    "classification",
    "clustering",
...
GATES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "minimum_profile": "explore",
        "applies": any_feature(
            "constructive_search", "beam_search", "learned_policy", "rolling_policy"
        ),
        "reason": "Constructive or policy search must prevent illegal actions before evaluation.",
    },
    {
    ...
    },
    {
        ...
        "minimum_profile": "candidate",
        "applies": any_feature("adaptive_search", "metaheuristic", "repeated_tuning"),
    ...
    },
    {
        ...
        "minimum_profile": "candidate",
        "applies": any_feature(
            "exact_simulator", "surrogate_dynamics", "discrete_event", "simulation_search"
        ),
...
def resolve(context: dict[str, Any]) -> dict[str, Any]:
    stage = normalize_stage(context.get("workflow_stage", context.get("workflow_profile", "explore")))
    ...
    for gate in GATES:
        ...
        elif STAGE_ORDER[normalize_stage(gate["minimum_profile"])] <= level:
            required.append(decision)
        else:
            decision["required_from_stage"] = normalize_stage(gate["minimum_profile"])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Persist --stage into the supplied project-state JSON after review.",
    )
    parser.add_argument(
        "--stage",
        "--profile",
        dest="stage",
        choices=(*STAGE_ORDER, *LEGACY_STAGE_ALIASES),
        help="Use explore, validate, or deliver; legacy values are normalized.",
    )
    parser.add_argument(
        "--task-family",
        choices=(
...
def main() -> int:
    ...
    try:
        ...
        stage = normalize_stage(
            args.stage
            or state.get("workflow_stage")
            or state.get("workflow_profile", "explore")
        )
        ...
        if args.update_project_state:
            if args.project_state is None or args.stage is None:
                raise ValueError(
                    "--update-project-state requires --project-state and --stage"
                )
