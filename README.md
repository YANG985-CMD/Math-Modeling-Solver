...
![Tests](https://img.shields.io/badge/Tests-26%2F26%20passing-16A34A)
...
### v2.2.0 - CUMCM 2026 LaTeX 论文生产与格式审查（2026-07-19）

- 新增独立编写的 `ctexart` 国赛论文模板，不复制来源和许可不明确的社区 `.cls` 文件。
- 固化 A4、四边 25 mm、摘要首页、页脚居中页码、无目录、匿名化、附录支撑材料清单等 2026 格式约束。
- 新增不依赖 Perl/`latexmk` 的 XeLaTeX 多轮编译脚本，保存输入/PDF 哈希、工具版本、命令和逐轮日志。
- 新增 CUMCM 格式审查器，检查页边距、目录、摘要顺序、缺图、缺代码、标签与引用、占位符、A4 PDF、20 MiB 文件限制和身份关键词。
- 新增 4 项回归测试，并要求最终 PDF 逐页视觉检查；字体、字号、行距和颜色仍作为风格选择，不冒充官方硬性规范。

### v2.1.0 - 结构真实性与候选晋级门（2026-07-19）
...
报告从 `assets/templates/candidate-validation-template.json` 创建。主指标通过后，仍需核对可行性、语义、有效支持、结构真实性和鲁棒性；多目标和多阶段模型还要分别检查覆盖与不确定性传播。
...
<!-- skill-provenance:v1;owner=YANG985-CMD;id=YANG985-CMD-MMS-2026-v9;path=README.md;sha256=bc3408eeb0d7bca2e0ac4119fe6e87e1ab9707cd8baa5c5020c29680ab13ec02;pub=0ofp8dKKJWMQK0LUC4dZDC8cynCRQlggy7cVeq7NfBo=;sig=yXgZ77gFOQPk4p_zCCZfpvvG9KA4BW_x83vfU-YU-Oz7d8QPIzpxivCR8CLKvjc2f9LlVi6FJKno10hFa0LlAA== -->
