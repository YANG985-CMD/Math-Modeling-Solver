...
![Tests](https://img.shields.io/badge/Tests-36%2F36%20passing-16A34A)
...
- 新增冻结结果自动生成论文表格，以及 Pandoc→OMML、Word 渲染逐页检查门。
...
python scripts/init_modeling_project.py D:\modeling\problem-a `
  --mode formal --workflow-profile explore --questions 3 --delivery-profile word-only
```
...
python scripts/resolve_required_gates.py `
  ...
  --delivery-profile word-only --out audit/gate-requirements.json
```

探索阶段只保留基线、语义、可运行性和硬约束；候选阶段再启用精确仿真、实验预算、鲁棒性等门；Word 公式和逐页检查延迟到交付阶段。
...
Word 数学公式必须由 Pandoc 转成原生 OMML；生成后用 Microsoft Word 渲染并逐页检查，审计会核对 `m:oMath`、DOCX 哈希和逐页复核清单。
...
<!-- skill-provenance:v1;owner=YANG985-CMD;id=YANG985-CMD-MMS-2026-v13;path=README.md;sha256=40f2ccab39cd38a5f7ee635823ee68d661c449824cdaf8ebf0ba0e5e8d9854d8;pub=0ofp8dKKJWMQK0LUC4dZDC8cynCRQlggy7cVeq7NfBo=;sig=KtZcBBR0pKNWxRDAHJ5MyKTHFOI5nyvB2qmPMZ3cX1hAjRvqp5DHLxPkc6JbUSaL53GI-qdqaY4NO003D_DGCg== -->
