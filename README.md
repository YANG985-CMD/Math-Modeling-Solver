...
[![CI](https://github.com/YANG985-CMD/Math-Modeling-Solver/actions/workflows/ci.yml/badge.svg)](https://github.com/YANG985-CMD/Math-Modeling-Solver/actions/workflows/ci.yml)

面向数学建模竞赛的端到端 Agent Skill。完整使用时可按需安装三个专业 Skill：证据审计、论文级绘图和 CUMCM 论文交付。
...
## 新版 Skill：Math Modeling Solver

`Math Modeling Solver` 是一个面向数学建模竞赛的证据驱动 Skill：它把题目拆解、数据审计、模型选择、可执行求解、结果验证、论文级绘图和 CUMCM 论文交付连接成一条可复现链路。

它不是四个互不相关的提示词集合，而是“一个总控入口 + 三个按需触发的专业入口”。完整赛题由总控负责协调；图表、证据审计或论文排版等独立任务只加载对应专业 Skill，减少无关上下文和门禁干扰。

| Skill 入口 | 主要职责 | 适合什么时候调用 |
| --- | --- | --- |
| `$math-modeling-solver` | 完整拆题、选模、求解、验证和交付协调 | 从一道题开始做完整建模方案 |
| `$audit-modeling-evidence` | 数据、实验、候选晋级、复现和 claim-to-evidence 审计 | 已有数据/代码/结果，需要判断是否可信 |
| `$build-modeling-figures` | 定量图、模型架构图、矢量导出和最终尺寸 QA | 主要任务是做图或检查图包 |
| `$deliver-cumcm-paper` | 冻结数字、CUMCM LaTeX/PDF 编译、格式预检和逐页检查 | 主要任务是生成或提交竞赛论文 |

根目录总控 Skill 保留原安装路径；三个专业 Skill 可单独安装，也可通过安装器一起部署。

| Skill | 主要职责 |
| --- | --- |
| `audit-modeling-evidence` | 数据、实验、候选晋级、复现和 claim-to-evidence 审计 |
| `build-modeling-figures` | 定量图、模型架构图、矢量导出和最终尺寸 QA |
| `deliver-cumcm-paper` | 冻结数字、CUMCM LaTeX/PDF 编译、格式预检和逐页检查 |

安装方式见下方“安装”一节；完整工作流可选配 [MATLAB 官方 MCP](https://github.com/matlab/matlab-mcp-server)，但 Python-only 任务不需要它。

## 版本更新日志

### v2.2.0 - 精确触发与专业 Skill pack（2026-07-20）

- 保留 `math-modeling-solver` 作为完整建模总控，新增 `audit-modeling-evidence`、`build-modeling-figures` 和 `deliver-cumcm-paper`。
- 将数据模式、工作流阶段、结果状态和交付配置明确为四个正交概念；证据门只作为检查结果。
- 将动作掩码、代理仿真一致性、实验家族预算等条件规则移出全局规则，只在适用任务中启用。
- 增加自包含专业 Skill 资源同步检查、统一依赖声明和 Windows/Ubuntu CI。
...
```bash
git clone https://github.com/YANG985-CMD/Math-Modeling-Solver.git /tmp/Math-Modeling-Solver
uv run --project /tmp/Math-Modeling-Solver python \
  /tmp/Math-Modeling-Solver/scripts/install_skill_pack.py
```
...
python scripts/init_modeling_project.py D:\modeling\problem-a `
  --mode formal --workflow-stage explore --questions 3 --delivery-profile cumcm-latex
```

生成的工作区包含问题契约、数据审计、方法决策、结果冻结、图表契约、论文契约、主张—证据账本和复现清单。工作流阶段只决定当前启用哪些门；数据模式为 `formal/demo`，缺少输入时由证据门返回 `blocked`。
...
| 维度 | 可选值 | 含义 |
| --- | --- | --- |
| `data_mode` | `formal` / `demo` | 数据真实性 |
| `workflow_stage` | `explore` / `validate` / `deliver` | 当前工作阶段 |
| `result_status` | `draft` / `validated` / `frozen` / `manuscript` | 结果能否支持下一阶段 |
| `delivery_profile` | `paper-bundle` / `cumcm-latex` / `code-only` / `custom` | 最终交付内容 |

证据门返回 `pass`、`warn` 或 `blocked`；`blocked` 表示缺少必要输入或授权，不是一个可以绕过检查的工作流快捷方式。`Intake`、`Method`、`Computation`、`Evidence` 和 `Manuscript` 是检查门，不是第五套状态机。
...
```text
使用 $audit-modeling-evidence 检查这个时间序列方案是否存在数据泄漏，
设计滚动验证，并把论文中的预测结论映射到真实结果表。
```

```text
使用 $audit-modeling-evidence 检查 TOPSIS 排名为什么不稳定，
设计权重扰动实验，并判断是否需要组合评价模型。
```

```text
使用 $build-modeling-figures 根据已有结果重构论文图表。
每张图先确定一个主结论，再生成 SVG、PDF、PNG、灰度图和 QA 报告。
```

```text
使用 $deliver-cumcm-paper 生成并审查 CUMCM LaTeX 论文。
...
```text
math-modeling-solver/
├─ SKILL.md                       # 端到端总控，兼容原安装方式
├─ agents/openai.yaml
├─ audit-modeling-evidence/       # 可单独安装的证据审计 Skill
├─ build-modeling-figures/        # 可单独安装的绘图 Skill
├─ deliver-cumcm-paper/           # 可单独安装的论文交付 Skill
├─ scripts/                       # 根目录兼容入口与同步/安装工具
├─ references/                    # 根目录源文件，按任务渐进加载
├─ benchmarks/                    # 外部链接、哈希、响应模板和隔离评分契约
├─ assets/                        # 算法、模板、LaTeX 和 README 展示资产
├─ pyproject.toml                 # 统一 Python 依赖
├─ .github/workflows/ci.yml       # Windows/Ubuntu 回归测试
└─ tests/
```
...
三个专业 Skill 需要分别执行同一校验命令。检查其自包含资源是否仍与根目录源文件同步：

```powershell
uv run python scripts/sync_specialist_skills.py --check
```

本次结构变更后，旧的 Ed25519 清单不再代表当前文件。发布前使用作者保存的私钥重新签名，并把四个 `SKILL.md` 与 README 都列为 marker file：

```powershell
uv run --with cryptography python scripts/provenance_watermark.py stamp `
  --root . --private-key <PRIVATE_KEY_PATH> --owner YANG985-CMD `
  --watermark-id <NEW_WATERMARK_ID> `
  --repository https://github.com/YANG985-CMD/Math-Modeling-Solver `
  --marker-file SKILL.md `
  --marker-file audit-modeling-evidence/SKILL.md `
  --marker-file build-modeling-figures/SKILL.md `
  --marker-file deliver-cumcm-paper/SKILL.md `
  --marker-file README.md
```
